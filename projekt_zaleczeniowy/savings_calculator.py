...
def oswiadczenie_wydatkow():
    ...
    while True:
        ...
        miesiac_wydatku = int(input("Podaj miesiąc wydatku (1-12): "))
        wydatki['planowane'].append((kwota, rok_wydatku, miesiac_wydatku))
    return wydatki
...
def oblicz_czas_oszczedzania(osoby, wydatki, cel):
    ...
    miesieczne_oszczednosci = miesieczny_dochod - miesieczne_wydatki
    # Sprawdzenie, czy miesięczne oszczędności są dodatnie
    if miesieczne_oszczednosci <= 0:
        print("Twoje wydatki przekraczają dochody lub są równe dochodom. Nie możesz oszczędzać.")
        return -1

    while current_savings < cel:
        miesieczne_oszczednosci = miesieczny_dochod - miesieczne_wydatki

        # Odjęcie kwartalnych wydatków co 3 miesiące
        if (month_counter + 1) % 3 == 0:
            miesieczne_oszczednosci -= wydatki['kwartalne']

        # Odjęcie rocznych wydatków co 12 miesięcy
        if (month_counter + 1) % 12 == 0:
            miesieczne_oszczednosci -= wydatki['roczne']

        # Uwzględnienie planowanych wydatków
        for wydatek in wydatki['planowane']:
            if wydatek[1] == start_date.year and wydatek[2] == start_date.month:
        ...
        # Sprawdzenie, czy oszczędności są większe niż cel
        if current_savings >= cel:
            break
